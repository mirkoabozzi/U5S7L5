...
public class ReservationsController {
    ...
    public Reservation saveReservations(@RequestBody @Validated ReservationsDTO payload, @AuthenticationPrincipal User authenticatedUser, BindingResult validation) {
        if (validation.hasErrors()) {
            String msg = validation.getAllErrors().stream().map(error -> error.getDefaultMessage()).collect(Collectors.joining());
            throw new BadRequestException("Payload error: " + msg);
        } else {
            return reservationsService.saveReservations(payload, authenticatedUser.getId());
        }
    }
}
